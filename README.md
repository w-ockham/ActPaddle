...
## ActPaddleを設定する
ActPaddleとPCを接続する方法には「USBシリアル接続」と「WiFi接続」があります。
...
最後に後述する[myACTの設定](#myACTの設定)の接続方法を「シリアル」に設定してください。

### WiFiで接続する
ActPaddleはiPhoneのテザリングやモバイルルータに接続することも可能です。

Android(Android10以降)の場合はActPaddleをアクセスポイントとして接続することでWiFi接続することができます[^1]。Androidで使う場合は、本節の説明は飛ばして[使ってみよう](#つかってみよう)の節を御覧ください。

まず最初に接続したいiPhoneやWiFiルータをアクセスポイントとして起動して下さい。iPhoneの場合は「設定」→「インターネット共有」→「ほかの人の接続を許可」でテザリング用のアクセスポイントが有効になります。

次にActPaddleをモバイルバッテリ等につなぎ起動します。USBケーブルを接続すると周りにあるアクセスポイントを検索（LEDが赤色に点灯します）後、「初期設定用アクセスポイント」として稼働します（LEDが消灯します）。
...
なおWiFiの接続状況に応じてATOMのLEDの色が以下のように変わります。
|LED色 | 接続状態　|
|:--|:--|
|赤|周囲のアクセスポイントを検索中|
|白|アクセスポイントに接続中|
...
[^1]:ActPaddleがマルチキャストDNSでIPアドレスを通知しているためテザリング等の環境でも名前でアクセスすることができます。Androidではテザリングのアクセスポイントに接続した際にアクセスポイントがマルチキャストDNSで告知された名前を認識しないため接続ができません。このためActPaddleをアクセスポイントとして接続することでmDNSでの名前解決を行います。

# 使ってみよう
## シリアル接続/WiFi接続(Android以外)の場合
...
## WiFi接続(Android)の場合
スマートフォンがインターネット環境にアクセスできる状態で、アクティベーション先を選択し、パドルダイアログまで表示してください。

次に、この状態でWiFi設定を開き、アクセスポイント`ActPaddle`を選択し接続します。パドルダイアログに所定のメッセージを入力したあと`SEND`ボタンを押すとメッセージが送出されます。

メッセージ録音後は再びWiFi設定を開き、アクセスポイント`ActPaddle`との接続を切断してください。
